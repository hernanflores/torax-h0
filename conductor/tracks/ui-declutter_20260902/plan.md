...
- [x] Task: Anotar el 16 → 12 en la Pre Spec y en `product.md` (FR9) [98ad6dd]
  - [x] Nota fechada 2026-09-02 en `Pre Spec Torax H-0.md`: doce Tracks por
  ...
  - [x] Nota equivalente en `conductor/product.md`, en el Core Model y en el MVP
        Scope.
  - [x] Dejar dicho que **el modelo no cambia de forma**: doce es un límite de
...
- [x] Task: `Pattern` guarda doce (FR1) [3d979c5]
  - [x] Tests (Red): recorrer los doce huecos; `track(at: 11)` devuelve valor y
        `track(at: 12)` devuelve `nil`; `Pattern.init()` asigna Track N → canal N
        para 1–12; `_isPOD(Pattern.self)` sigue cierto.
  - [x] Implementación (Green): `trackCount = 12`, tupla de doce, `init()`
        acotado.
  - [x] Verificar que `Pattern.initial` sigue sonando igual — material solo en el
        Track 1.
  - [x] Cobertura `Engine` ≥90%.
- [x] Task: Anillos, playhead y posición de Cycle derivan de doce (FR2) [3d979c5]
  - [x] Tests (Red): `RingStack` produce doce bandas y el ancho de banda es el
        esperado para `trackCount − 1`; `Playhead` y `CyclePosition` devuelven
        doce entradas.
  - [x] Implementación: comprobar que **no hace falta tocar nada** — si algún
        sitio tiene 16 escrito a mano, sale aquí y se sustituye por
        `Pattern.trackCount`.
- [x] Task: El scheduler y el transporte con doce (FR1) [3d979c5]
  - [x] Tests (Red): `PatternScheduler` asigna doce schedulers; `Transport` apaga
...
- [x] Task: Los step buttons 13–16 no seleccionan Track (FR3) [3d979c5]
  - [x] Tests (Red): el step button 12 selecciona el Track 12; los 13 a 16 no
        publican selección y no son error. `ControlMapping.declaredNumbers` sigue
        declarando dieciséis.
  - [x] Implementación (Green): acotar en la selección de Track, no en el mapeo —
...
- [x] Task: Doce pastillas, un número cada una (FR4) [d582038]
  - [x] Quitar el canal del `label` del botón y dejar el número centrado, con los
        tres estados intactos.
  - [x] La fila pasa de dieciséis a doce posiciones, derivadas de
        `Pattern.trackCount` y no de un literal.
  - [x] Actualizar la documentación de la vista: la razón de «una fila y no dos»
...
- [x] Task: La pantalla Track pierde su fila `Channel` (FR7) [d582038]
  - [x] Retirar `channelRow`, `selectedChannel` y los parámetros `channels` /
        `onChannelChange` de `TrackSelectorView`.
  - [x] `ContentView` deja de pasárselos; `model.channels` y `model.setChannel`
...
- [x] Task: `3 · MIDI` deja de estar en discontinuo (FR5) [38d846f]
  - [x] Añadir el caso a `Screen` con su etiqueta; `unavailableScreens` queda en
        `["4 · Banks", "5 · Tracks"]`.
  - [x] Pantalla vacía con su título, para que el cambio de navegación se
        verifique aparte del contenido.
- [x] Task: La asignación de canal, los doce a la vez (FR6) [f12fa56]
  - [x] Vista nueva en `App`: doce filas `Track N` con selector de canal 1–16,
        usando `Brutalist` y `Typography` — sin estilo propio (NFR5).
  - [x] Cablear a `model.channels` y a un `setChannel(_:forTrack:)`, que hoy solo
        sabe del Track seleccionado.
  - [x] Si esa función necesita lógica —acotar índice, validar canal—, va a `MIDI`
...
- [x] Task: Rejilla `12-tracks-cycles` (FR8) [df015f6]
  - [x] Renombrar el caso y su cadena; `trackCount: 12`.
  - [x] Actualizar las referencias en los `device-verification.md` de las
        rebanadas 2 y 3 y en `workflow.md`, que nombran la rejilla vieja.
  - [x] **No se mide** — suspendido el 2026-09-02. La tarea deja la herramienta
